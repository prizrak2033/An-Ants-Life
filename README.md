...
- `--profile expedition-alpha` uses a named save slot in `.an_ants_life_saves/`.
- `--list-profiles` shows available named save slots and their summaries.
- `--save-file /path/to/save.json` overrides the profile system with an explicit save path.
...
The game now persists colony progress to `.an_ants_life_saves/default.json` by default.
